...
- Past searches: ${searches.slice(-10).join(", ")||"none yet"}
- Products explored: ${prods.slice(-8).map(p=>p.name).join(", ")||"none yet"}
...
1. "dealsForYou": 3-4 products with deals relevant to this user's interests and searches (include ALL interest areas, not just the last search)
2. "trending": 3-4 trending search queries similar shoppers are doing, with shopperCount
3. "popularPurchases": 3-4 products popular with similar shoppers
4. "becauseYouSearched": [{"query":"original search","products":[1-2 related products]}] for the last 2-3 searches
...
Each product: {"name":"","price":0,"rating":4.5,"reviews":100,"retailer":"","category":"","emoji":"","url":"","deal":true,"dealPct":10,"whyRecommended":""}`;
  ...
  const PC=({p,sm})=>(<div onClick={()=>open(p)} style={{background:"#fff",borderRadius:14,border:"1px solid #f0f0f0",overflow:"hidden",cursor:"pointer",minWidth:sm?160:undefined,maxWidth:sm?160:undefined,flexShrink:0}}>
    <div style={{height:sm?100:130,background:"#f8f8f8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:sm?40:52,position:"relative"}}>{p.img}{p.deal&&<span style={{...s.badge("#ef4444"),position:"absolute",top:6,left:6,fontSize:10}}>-{p.dealPct}%</span>}<div onClick={e=>{e.stopPropagation();togSave(p.id)}} style={{position:"absolute",bottom:6,right:6,color:saved.includes(p.id)?"#ef4444":"#ccc"}}><I.Heart s={15} f={saved.includes(p.id)}/></div></div>
  ...
  return(
    <div style={s.app}>
      ...
      {pg==="activity"&&(<>
        ...
        {viewedP.length===0&&savedP.length===0&&buyList.length===0&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"60px 20px",gap:12,textAlign:"center"}}><div style={{fontSize:48}}>📋</div><div style={{fontSize:16,fontWeight:700}}>No activity yet</div></div>}
      </>)}
