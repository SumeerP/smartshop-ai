...
export default function App(){
  ...
  useEffect(()=>{
    window.__smartshopMCP={
      queue:[],
      status:{},
      getQueue:()=>window.__smartshopMCP.queue,
      notify:(productId,status,details)=>{
        window.__smartshopMCP.status[productId]={status,details,ts:Date.now()};
        window.dispatchEvent(new CustomEvent('mcp-update',{detail:{productId,status,details}}));
      },
      addingToCart:(pid)=>window.__smartshopMCP.notify(pid,'adding_to_cart',{message:'Navigating to Amazon...'}),
      addedToCart:(pid)=>window.__smartshopMCP.notify(pid,'added_to_cart',{message:'Product added to cart!'}),
      checkingOut:(pid)=>window.__smartshopMCP.notify(pid,'checking_out',{message:'Proceeding to checkout...'}),
      complete:(pid,msg)=>window.__smartshopMCP.notify(pid,'completed',{message:msg||'Order placed successfully!'}),
      fail:(pid,msg)=>window.__smartshopMCP.notify(pid,'error',{message:msg||'Something went wrong'})
    };
    const handler=(e)=>setMcpStatus(p=>({...p,[e.detail.productId]:{status:e.detail.status,details:e.detail.details}}));
    window.addEventListener('mcp-update',handler);
    return()=>window.removeEventListener('mcp-update',handler);
  },[]);
...
Use realistic specs for this product type. Keep description under 50 words.`;
  ...
  // MCP Add to Cart — queues product for Chrome MCP to process
  const mcpBuy=useCallback((p)=>{
    if(!window.__smartshopMCP)return;
    const item={id:p.id,name:p.name,asin:p.asin||'',price:p.price,url:p.url,retailer:p.retailer,ts:Date.now()};
    window.__smartshopMCP.queue.push(item);
    window.__smartshopMCP.status[p.id]={status:'adding_to_cart',ts:Date.now()};
    setMcpStatus(prev=>({...prev,[p.id]:{status:'adding_to_cart'}}));
  },[]);
      ...
      {pg==="product"&&sel&&(()=>{const p=sel,sv=saved.includes(p.id),bt=buys.some(x=>x.pid===p.id),sim=prods.filter(x=>x.cat===p.cat&&x.id!==p.id).slice(0,4);const hasAsin=!!p.asin;const dImg=getImg(p);const mSt=mcpStatus[p.id];return(<div style={{paddingBottom:80,overflowY:"auto",height:"100vh"}}>
        ...
        <div style={{padding:20}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:18}}>
            {hasAsin&&(()=>{const busy=mSt&&mSt.status!=='error';const done=mSt?.status==='completed';
              const bgColor=done?"#16a34a":busy?"#ff9900":"#ff9900";
              const label=done?"Order Placed!":mSt?.status==='checking_out'?"Checking Out...":mSt?.status==='added_to_cart'?"Added to Cart...":mSt?.status==='adding_to_cart'?"Adding to Cart...":"Buy from Amazon";
              return <button onClick={()=>{if(!busy)mcpBuy(p);}} disabled={busy&&!done} style={{...s.btn(),background:bgColor,display:"flex",alignItems:"center",justifyContent:"center",gap:10,fontSize:15,padding:"14px 20px",border:"none",position:"relative",overflow:"hidden"}}>
                {busy&&!done&&<span style={{display:"inline-block",width:16,height:16,border:"2.5px solid rgba(255,255,255,0.4)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>}
                {done&&<I.Check s={18}/>}
                {!busy&&<I.CartPlus s={18}/>}
                {label}
              </button>;
            })()}

            {/* Live status tracker */}
            {hasAsin&&mSt&&<div style={{border:"1px solid #f0f0f0",borderRadius:12,overflow:"hidden",background:"#fff"}}>
              {[
                {key:'adding_to_cart',icon:'🛒',text:'Adding to cart',active:mSt.status==='adding_to_cart'},
                {key:'added_to_cart',icon:'✅',text:'Added to cart',active:mSt.status==='added_to_cart'},
                {key:'checking_out',icon:'💳',text:'Checking out',active:mSt.status==='checking_out'},
                {key:'completed',icon:'🎉',text:'Order complete',active:mSt.status==='completed'},
              ].map((step,i)=>{
                const steps=['adding_to_cart','added_to_cart','checking_out','completed'];
                const ci=steps.indexOf(mSt.status);const si=steps.indexOf(step.key);
                const past=si<ci;const current=si===ci;const future=si>ci;
                const isError=mSt.status==='error';
                return <div key={step.key} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderBottom:i<3?"1px solid #f5f5f5":"none",opacity:future?0.35:1,background:current?"#fffbeb":"transparent"}}>
                  <span style={{fontSize:16,filter:future?"grayscale(1)":"none"}}>{isError&&current?"❌":past?"✅":step.icon}</span>
                  <span style={{fontSize:13,fontWeight:current?600:400,color:past?"#16a34a":current?"#92400e":"#888",flex:1}}>{step.text}</span>
                  {current&&!isError&&<span style={{display:"inline-block",width:12,height:12,border:"2px solid #f59e0b",borderTopColor:"transparent",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>}
              ...
              {mSt.status==='completed'&&mSt.details?.message&&<div style={{padding:"10px 14px",background:"#f0fdf4",color:"#16a34a",fontSize:12,fontWeight:500}}>{mSt.details.message}</div>}
            </div>}
