...
Use realistic specs for this product type. Keep description under 50 words.`;
  ...
  const mcpAddToCart=useCallback((p)=>{
    if(!window.__smartshopMCP)return;
    const item={id:p.id,name:p.name,asin:p.asin||'',price:p.price,url:p.url,ts:Date.now()};
    window.__smartshopMCP.queue.push(item);
    window.__smartshopMCP.status[p.id]={status:'queued',ts:Date.now()};
    setMcpStatus(prev=>({...prev,[p.id]:{status:'queued'}}));
  },[]);
      ...
      {pg==="product"&&sel&&(()=>{const p=sel,sv=saved.includes(p.id),bt=buys.some(x=>x.pid===p.id),sim=prods.filter(x=>x.cat===p.cat&&x.id!==p.id).slice(0,4);const hasAsin=!!p.asin;const dImg=getImg(p);const mSt=mcpStatus[p.id];return(<div style={{paddingBottom:80,overflowY:"auto",height:"100vh"}}>
        ...
        <div style={{padding:20}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:18}}>
            ...
            {hasAsin&&<button onClick={()=>mcpAddToCart(p)} disabled={mSt?.status==='queued'||mSt?.status==='processing'||mSt?.status==='completed'} style={{...s.btn(),background:mSt?.status==='completed'?"#16a34a":mSt?.status==='queued'||mSt?.status==='processing'?"#6366f1":"#232f3e",display:"flex",alignItems:"center",justifyContent:"center",gap:8,opacity:mSt?.status==='queued'||mSt?.status==='processing'?0.8:1}}>
              {mSt?.status==='completed'?<><I.Check s={16}/> Added to Cart via AI</>:mSt?.status==='queued'||mSt?.status==='processing'?<><span style={{display:"inline-block",width:14,height:14,border:"2px solid #fff",borderTopColor:"transparent",borderRadius:"50%",animation:"spin 1s linear infinite"}}/>AI Processing...</>:<><span style={{fontSize:14}}>🤖</span> Add to Cart via AI</>}
            </button>}
            {mSt&&<div style={{padding:"8px 12px",borderRadius:8,fontSize:12,background:mSt.status==='completed'?"#f0fdf4":mSt.status==='error'?"#fef2f2":"#eef2ff",color:mSt.status==='completed'?"#16a34a":mSt.status==='error'?"#dc2626":"#4f46e5"}}>
              {mSt.status==='queued'&&"⏳ Queued — Tell Claude to process your cart"}
              {mSt.status==='processing'&&"🤖 AI is adding to your Amazon cart..."}
              {mSt.status==='completed'&&`✅ ${mSt.details?.message||"Added to Amazon cart!"}`}
              {mSt.status==='error'&&`❌ ${mSt.details?.message||"Failed — try again"}`}
            </div>}
