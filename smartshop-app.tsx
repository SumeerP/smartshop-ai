...
{"message":"Conversational response with **bold**","products":[{"name":"Name","price":29.99,"rating":4.5,"reviews":1234,"retailer":"Amazon","category":"Electronics","emoji":"🎧","url":"https://amazon.com/dp/B0XXXXX","asin":"B0XXXXX","imageUrl":"https://m.media-amazon.com/images/I/XXXXX._AC_SX200_.jpg","deal":false,"dealPct":0,"whyRecommended":"Reason"}],"followUpQuestion":"Question","searchQueries":["q1","q2"]}
Rules: 2-5 real products, real prices, real retailers. Use web search for current data. Appropriate emoji. Always include followUpQuestion and searchQueries. If user has a profile, tailor recommendations to their preferences. IMPORTANT: For Amazon products, include a real ASIN and a real Amazon product image URL (m.media-amazon.com). For other retailers, omit asin and imageUrl.`;
}
...
function parseAI(raw){
  try{
    ...
    return{msg:j.message||"Here are recommendations:",products:(j.products||[]).map((p,i)=>({id:`ai-${Date.now()}-${i}`,name:p.name||"Product",price:p.price||0,rating:p.rating||4.0,reviews:p.reviews||0,retailer:p.retailer||"Online",cat:p.category||"General",img:p.emoji||"🛍️",url:p.url||"#",asin:p.asin||"",imageUrl:p.imageUrl||"",deal:!!p.deal,dealPct:p.dealPct||0,why:p.whyRecommended||""})),followUp:j.followUpQuestion||null,suggestions:j.searchQueries||[]};
  }catch(e){
...
Item format: {"name":"...","price":N,"rating":4.5,"reviews":N,"retailer":"...","category":"...","emoji":"🎧","url":"#","deal":false,"dealPct":0,"whyRecommended":"..."}`;
    ...
    callAI([{role:"user",content:prompt}],sys,{webSearch:false,maxTokens:1500}).then(raw=>{
      ...
      const mkP=(arr)=>(arr||[]).map((p,i)=>({id:`home-${Date.now()}-${i}-${Math.random().toString(36).slice(2,6)}`,name:p.name||"",price:p.price||0,rating:p.rating||4.0,reviews:p.reviews||0,retailer:p.retailer||"Online",cat:p.category||"General",img:p.emoji||"🛍️",url:p.url||"#",asin:p.asin||"",imageUrl:p.imageUrl||"",deal:!!p.deal,dealPct:p.dealPct||0,why:p.whyRecommended||""}));
...
Replace example values with real products for: ${recentSearches}. User budget: ${user.budget}. Keep JSON valid, no trailing commas.`;
  ...
  useEffect(()=>{
    ...
    if(homeHasData.current&&searches.length-homeDataSearchCount.current<3)return;
    if(homeFeedTimer.current)clearTimeout(homeFeedTimer.current);
    const delay=homeHasData.current?30000:5000;
...
Mix ratings (mostly 4-5, one 3). Realistic names. Keep bodies under 60 words.`;
    callAI([{role:"user",content:msg}],sys,{webSearch:false,maxTokens:800}).then(raw=>{
      let c=raw.replace(/```json\s*/gi,"").replace(/```\s*/gi,"").trim();
      const si=c.indexOf("["),ei=c.lastIndexOf("]");
...
Use realistic specs for this product type. Keep description under 50 words.`;
    callAI([{role:"user",content:msg}],sys,{webSearch:false,maxTokens:600}).then(raw=>{
      let c=raw.replace(/```json\s*/gi,"").replace(/```\s*/gi,"").trim();
      const si=c.indexOf("{"),ei=c.lastIndexOf("}");
      c=c.slice(si,ei+1).replace(/,\s*([}\]])/g,'$1');
  ...
  const PC=({p,sm})=>{const inCmp=compareIds.includes(p.id);const[imgErr,setImgErr]=useState(false);return(<div onClick={()=>open(p)} style={{background:"#fff",borderRadius:14,border:inCmp?"2px solid #7c3aed":"1px solid #f0f0f0",overflow:"hidden",cursor:"pointer",minWidth:sm?160:undefined,maxWidth:sm?160:undefined,flexShrink:0}}>
    <div style={{height:sm?100:130,background:"#f8f8f8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:sm?40:52,position:"relative"}}>{p.imageUrl&&!imgErr?<img src={p.imageUrl} alt={p.name} onError={()=>setImgErr(true)} style={{height:"100%",width:"100%",objectFit:"contain",padding:8}}/>:p.img}{p.deal&&<span style={{...s.badge("#ef4444"),position:"absolute",top:6,left:6,fontSize:10}}>-{p.dealPct}%</span>}<div onClick={e=>{e.stopPropagation();togCompare(p.id)}} style={{position:"absolute",top:6,right:6,width:26,height:26,borderRadius:6,background:inCmp?"#7c3aed":"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",cursor:"pointer"}}>{inCmp?<I.Check s={14}/>:<I.Compare s={14}/>}</div><div onClick={e=>{e.stopPropagation();togSave(p.id)}} style={{position:"absolute",bottom:6,right:6,color:saved.includes(p.id)?"#ef4444":"#ccc"}}><I.Heart s={15} f={saved.includes(p.id)}/></div></div>
  ...
  return(
    <div style={s.app}>
      ...
      {/* PRODUCT */}
      {pg==="product"&&sel&&(()=>{const p=sel,sv=saved.includes(p.id),bt=buys.some(x=>x.pid===p.id),sim=prods.filter(x=>x.cat===p.cat&&x.id!==p.id).slice(0,4);const hasAsin=!!p.asin;return(<div style={{paddingBottom:80,overflowY:"auto",height:"100vh"}}>
        ...
        <div style={{background:"#f8f8f8",height:240,display:"flex",alignItems:"center",justifyContent:"center",fontSize:90}}>{p.imageUrl?<img src={p.imageUrl} alt={p.name} onError={e=>{e.target.style.display="none";e.target.nextSibling.style.display="block";}} style={{height:"100%",width:"100%",objectFit:"contain",padding:16}}/>:null}<span style={{display:p.imageUrl?"none":"block"}}>{p.img}</span></div>
        <div style={{padding:20}}>
          <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}><span style={{fontSize:12,color:"#888",background:"#f0f0f0",padding:"3px 10px",borderRadius:6}}>{p.cat}</span><span style={{fontSize:12,color:"#888",background:"#f0f0f0",padding:"3px 10px",borderRadius:6}}>{p.retailer}</span>{hasAsin&&<span style={{fontSize:11,color:"#ff9900",background:"#fff8ee",padding:"3px 10px",borderRadius:6,fontWeight:600}}>Amazon ASIN: {p.asin}</span>}</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:18}}>
            {hasAsin&&<a href={`https://www.amazon.com/gp/aws/cart/add.html?ASIN.1=${p.asin}&Quantity.1=1`} target="_blank" rel="noopener noreferrer" style={{...s.btn(),textDecoration:"none",background:"#ff9900",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><I.CartPlus s={16}/> Add to Amazon Cart</a>}
            <div style={{display:"flex",gap:10}}>
              {p.url&&p.url!=="#"?<a href={p.url} target="_blank" rel="noopener noreferrer" style={{...s.btn(hasAsin?"s":undefined),flex:2,textDecoration:"none"}}>{hasAsin?"View on":"Buy from"} {p.retailer} <I.ExtLink/></a>:<button style={{...s.btn(hasAsin?"s":undefined),flex:2}}>{hasAsin?"View on":"Buy from"} {p.retailer} <I.ExtLink/></button>}
